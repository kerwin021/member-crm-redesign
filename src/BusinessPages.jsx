...
function objectEntries(data) {
  if (Array.isArray(data)) return data.map((value, index) => [String(index + 1), value]);
  if (!data || typeof data !== "object") return [];
  return Object.entries(data);
}

function displayDatabaseValue(value) {
  if (value === null || value === undefined || value === "") return "数据库暂无";
...
function HighValuePage({ onToast, onAction, data }) {
  ...
  return (
    <section className="business-page">
      ...
      <Drawer open={!!selected} title={selected?.name || ""} subtitle={`${selected?.id || ""} · 高贡献会员画像`} onClose={() => setSelected(null)} footer={<><button className="outline-button" onClick={() => onToast("已记录跟进任务")}>创建跟进</button><button className="primary-button" onClick={() => onAction("发放优惠券")}><IconDiscount2 size={16} />发放专属权益</button></>}><div className="drawer-profile"><span>{selected?.name.slice(0,1)}</span><div><strong>{selected?.level}</strong><p>{selected?.store}</p></div></div><div className="drawer-metrics"><div><small>价值评分</small><strong>{selected?.score}</strong></div><div><small>累计贡献</small><strong>{selected?.value}</strong></div><div><small>订单数</small><strong>{selected?.orders}</strong></div></div><h3>关键标签</h3><div className="tag-cloud">{rowsFrom(selected?.tags).map((tag, index) => <Pill tone={["blue", "green", "orange", "purple"][index % 4]} key={tag}>{tag}</Pill>)}{!selected?.tags?.length && <small>数据库暂无会员标签</small>}</div><h3>AI 运营建议</h3><div className="drawer-insight"><IconSparkles size={20} /><p>数据库暂无该会员的 AI 运营建议，可通过微智 Claw 智能问答生成基于当前会员数据的建议。</p></div></Drawer>
    </section>
  );
}
...
function SegmentsPage({ onToast, data }) {
  ...
  const totalMembers = Number(data.dashboard?.summary?.totalMembers || data.memberCount || 0);
...
function ProductsPage({ onToast, data }) {
  ...
  const sellThrough = productStats.total ? ((Number(productStats.selling || 0) / Number(productStats.total)) * 100).toFixed(1) : "0.0";
...
const domainConfigs = {
  ...
  "domain-config": {
    title: "配置管理", subtitle: "统一维护组织权限、系统配置、业务规则和开放接口", icon: IconSettings, tone: "purple",
    modules: [["系统参数", "配置会员、订单、营销和安全通用参数", "参数配置"], ["组织权限", "维护组织、门店、员工、角色和审批", "管理权限"], ["消息与字典", "维护消息模板、业务字典和基础枚举", "管理模板"], ["开放接口", "管理应用、API 文档、接口权限和调用日志", "查看接口"], ["任务调度", "查看自动任务、执行记录和异常", "查看任务"], ["事件回调", "配置 Webhook、事件订阅和失败重试", "配置回调"]],
  },
};
