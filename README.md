...
- 用户数据：会员档案、分群、标签、商品、订单、经营洞察与单客模型 / LTV
...
- 入口：`用户数据 → 用户洞察 → 单客模型 / LTV`。
- 演示链路：公司 → 区域 → 门店 → 人群包 → 策略 → 任务 → 复盘。
- 每一级同时展示业务对象、关键指标、“最终呈现”和“怎么使用”，支持切换对象、返回上级、重新演示和导出演示摘要。
- 复盘页对比目标与实际指标，输出策略结论，并可将有效策略沉淀为后续复用模板。
- 业务节点、指标和复盘趋势均来自 MySQL 的 `analytics_ltv_nodes` 表，通过 `/api/app-data` 的 `ltvModel` 字段提供；前端没有 LTV 业务数据回退。
